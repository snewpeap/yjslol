...
let viewType = document.querySelectorAll('#day-group li');
[].forEach.call(viewType, function (view) {
...
let viewTypeDict = {"多选":0,"单选":1};
let positionNow = 0;
let changeType = false;
let viewTypeNow = "multiple";
let tenDaysRecord = [];
let day = 0;
let allData = [[],[],[],[],[]];
let allTime = [];
...
let option = {
    ...
    //         let tip='<div><p>时间：'+params[0].name+'</p></div>';
    //         for(let i = 0;i<10;i++){
    //             tip += '<p>'+params[i].seriesName+':'+(params[i].seriesIndex+1)+'</p>'
    //         }
    //         return tip;
    //     }
    // },
...
function draw() {
    ...
    option.legend.selectedMode = viewTypeNow;
    if(viewTypeNow === 'multiple'&&changeType){
        option.legend.selected = JSON.stringify(selected);
    }else{
        option.legend.selected = {};
    }
    changeType = false;
    option.legend.data = heroNames;
